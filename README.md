...
当前已验证的是**启动器自身更新**；下面才是 DSH 管理能力下一轮的明确工作顺序。每项完成前不会标记为正式稳定版。
...
2. **DSH 更新的真实验收**：在 Win10、Win11 分别覆盖国内镜像和官方源、取消、强制关闭、网络失败、更新成功、修复重装及服务短暂退出。重点验证中断后重开启动器不会误报运行，也不会破坏原可用 DSH。
3. **安装与卸载流程**：完成下载/安装 DSH、下载/安装 Node.js、卸载 DSH、卸载 Node.js 的真实环境测试；卸载仅在用户明确确认后执行，并默认保留 `DSH_HOME`、凭据和会话数据。
4. **发布与诊断收尾**：完善更新失败日志和一键导出诊断信息；后续新 Release 安装包随附 `LICENSE`，并继续保留 Gitee 主源、GitHub 备用源和 SHA-256 校验。

实现顺序上，优先完成第 1、2 项，再扩展安装/卸载功能；这样即使用户中途中止操作，也不会出现“界面显示正常、实际 DSH 文件或服务已失效”的状态。
